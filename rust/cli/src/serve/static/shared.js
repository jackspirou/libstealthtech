...
(function () {
  ...
  var logFormatters = {
    ...
    MuteState: function (v) {
      return "Mute: " + (v ? "on" : "off");
    },
    QuietMode: function (v) {
      return "Quiet Mode: " + (v ? "on" : "off");
    },
    Balance: function (v) {
      return "Balance: " + v + "/100";
    },
    Layout: function (v) {
      return "Layout: " + v;
    },
    CurrentInput: function (v) {
      return "Input: " + v;
    },
    Power: function (v) {
      return "Power: " + (v ? "on" : "standby");
    },
    CurrentSoundMode: function (v) {
      return "Sound Mode: " + v;
    },
    Covering: function (v) {
      return "Covering: " + v;
    ...
    SubwooferConnected: function (v) {
      return "Subwoofer: " + (v ? "connected" : "disconnected");
    },
    RearVolume: function (v) {
      return "Rear: " + v + "/30";
    },
  ...
  function addLogEntry(message) {
    ...
    var now = new Date();
    var time = now.toLocaleTimeString("en-US", { hour12: true }) + "." + String(now.getMilliseconds()).padStart(3, "0");

    entry.innerHTML =
      '<span class="log-time">' +
      time +
      '</span><span class="log-message">' +
      escapeHtml(formatLogMessage(message)) +
      "</span>";
  ...
  function setControlsEnabled(enabled, opts) {
    ...
    $$(".profile-btn-wrap .btn-option").forEach(function (btn) {
      btn.disabled = !enabled;
    });
    ...
    if (state.connected != null) {
      if (state.connected) {
        ...
        statusText.innerHTML =
          "Connected" + (state.name ? ' <span class="status-device-name">- ' + escapeHtml(state.name) + "</span>" : "");
  ...
  function renameProfile(oldName) {
    var m = showModal(
      "<h3>Rename Profile</h3>" +
        ...
        'value="' +
        escapeHtml(oldName) +
        '" maxlength="20" aria-label="Profile name">' +
  ...
  function showSettings() {
    ...
    var m = showModal(
      "<h3>Settings</h3>" +
        ...
        '<button class="btn btn-option settings-theme-btn' +
        (theme === "auto" ? " active" : "") +
        '" data-theme-val="auto">Auto</button>' +
        '<button class="btn btn-option settings-theme-btn' +
        (theme === "light" ? " active" : "") +
        '" data-theme-val="light">Light</button>' +
        '<button class="btn btn-option settings-theme-btn' +
        (theme === "dark" ? " active" : "") +
        '" data-theme-val="dark">Dark</button>' +
        "</div>" +
        "</div>" +
        '<div class="settings-section">' +
